...
The bottleneck in both cases is **CPU throughput, not RAM**. Modern LLMs require extremely high memory bandwidth to stream billions of parameters from RAM into compute units during each token generation step. CPUs have nowhere near the memory bandwidth of a GPU — a consumer GPU like an RTX 3090 delivers ~936 GB/s, while a high-end desktop CPU tops out around 50–100 GB/s. Even with ample RAM installed, the CPU simply cannot feed the model weights fast enough to generate tokens at an acceptable rate.
